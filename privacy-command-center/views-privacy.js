...
V['privacy/worstday'] = { title: 'Worst Day', render: function () {
  ...
  var cols = DESIGNS.map(function (x) {
    ...
    return '<div class="wd-col' + (x[0] === 'CURRENT' ? ' cur' : '') + '"><div class="mono small dim">' + x[0] + '</div><svg viewBox="0 0 240 240" width="100%" style="max-width:220px" role="group" aria-label="' + esc(x[1]) + ' blast radius ' + Math.round(rel * 100) + ' percent"><circle cx="120" cy="120" r="112" fill="none" stroke="#1f2835" stroke-dasharray="2 5"/><circle cx="120" cy="120" r="' + r.toFixed(1) + '" fill="' + (rel > 0.6 ? '#ff5d73' : rel > 0.25 ? '#ff8a5c' : rel > 0.02 ? '#f2c46d' : '#4fd8b6') + '" fill-opacity=".22" stroke="' + (rel > 0.6 ? '#ff5d73' : rel > 0.25 ? '#ff8a5c' : rel > 0.02 ? '#f2c46d' : '#4fd8b6') + '"/><text x="120" y="126" fill="#fff" font-size="26" font-weight="700" text-anchor="middle">' + Math.round(rel * 100) + '%</text></svg>' +
...
function miniDFD(sysIds, flows) {
  ...
  flows.forEach(function (f) { var a = pos[f.from], b = pos[f.to]; if (!a || !b) return; var ax = a[0] + 150, ay = a[1] + 15, bx = b[0], by = b[1] + 15, mx = (ax + bx) / 2; if (b[0] <= a[0]) { bx = b[0] + 150; mx = ax + 40; } var c = f.status === 'unknown' ? '#b39bff' : (f.flags || []).indexOf('purpose_change') >= 0 ? '#f2c46d' : f.boundary === 'third_party' ? '#ff8a5c' : '#5d6a7f'; s += '<g class="edge" data-ent="' + f.id + '" tabindex="0" role="button" aria-label="' + esc(P.name(f.id)) + '"><path class="hit" d="M' + ax + ',' + ay + ' C' + mx + ',' + ay + ' ' + mx + ',' + by + ' ' + bx + ',' + by + '"/><path class="vis" d="M' + ax + ',' + ay + ' C' + mx + ',' + ay + ' ' + mx + ',' + by + ' ' + bx + ',' + by + '" fill="none" stroke="' + c + '" stroke-width="' + (1 + f.tier * 0.5) + '"' + (f.status !== 'reviewed' ? ' stroke-dasharray="5 4"' : '') + '/></g>'; });
  nodes.forEach(function (n) { var p = pos[n], e = P.get(n); if (!e) return; var tp = e.type, col = tp === 'vendor' || tp === 'subprocessor' ? '#ff8a5c' : tp === 'model' ? '#e59cff' : '#8fb3d9'; var l = P.name(n); if (l.length > 21) l = l.slice(0, 20) + '…'; s += '<g class="node" data-ent="' + n + '" tabindex="0" role="button" aria-label="' + esc(P.name(n)) + '"><rect x="' + p[0] + '" y="' + p[1] + '" width="150" height="30" rx="8" fill="#121821" stroke="#2b3647"/><rect x="' + p[0] + '" y="' + p[1] + '" width="3.5" height="30" rx="2" fill="' + col + '"/><text x="' + (p[0] + 10) + '" y="' + (p[1] + 19) + '" fill="#e2e7ef" font-size="11">' + esc(l) + '</text></g>'; });
  return s + '</svg>';
}
P.miniDFD = miniDFD;
...
V['privacy/consent'] = { title: 'Consent', render: function () {
  ...
  [[0.1, '100 ms'], [1, '1 s'], [60, '1 min'], [3600, '1 h'], [86400, '1 d'], [604800, '7 d']].forEach(function (t) { var x = xs(t[0]); s += '<line x1="' + x + '" x2="' + x + '" y1="10" y2="' + (H - 24) + '" stroke="#1f2835"/><text x="' + x + '" y="' + (H - 8) + '" fill="#7c879b" font-size="10" text-anchor="middle">' + t[1] + '</text>'; });
  cs.forEach(function (c, i) {
    var y = 20 + i * rowH;
    s += '<g class="node" data-ent="' + c.id + '" tabindex="0" role="button" aria-label="' + esc(c.name) + '"><text x="' + (x0 - 10) + '" y="' + (y + 4) + '" fill="' + (c.p50 == null ? '#ff8a5c' : '#cfd6e1') + '" font-size="11.5" text-anchor="end">' + esc(c.name) + '</text>';
    if (c.p50 == null) s += '<line x1="' + x0 + '" x2="' + (W - 60) + '" y1="' + y + '" y2="' + y + '" stroke="#ff8a5c" stroke-dasharray="3 4" stroke-opacity=".6"/><text x="' + (W - 56) + '" y="' + (y + 4) + '" fill="#ff8a5c" font-size="10.5">never · ' + esc(c.mode) + '</text>';
    else s += '<line x1="' + xs(c.p50) + '" x2="' + xs(c.p99) + '" y1="' + y + '" y2="' + y + '" stroke="#4fd8b6" stroke-opacity=".5" stroke-width="2"/><circle cx="' + xs(c.p95) + '" cy="' + y + '" r="3" fill="none" stroke="#4fd8b6"/><circle cx="' + xs(c.p50) + '" cy="' + y + '" r="5" fill="#4fd8b6"/><text x="' + (xs(c.p99) + 8) + '" y="' + (y + 4) + '" fill="#7c879b" font-size="10">' + esc(c.mode) + '</text>';
    s += '</g>';
  });
  s += '</svg>';
...
function stateMachine() {
  return '<svg viewBox="0 0 420 170" width="100%" style="margin-top:12px" role="group" aria-label="Consent state machine: unknown, granted, revoked, expired"><defs><marker id="smA" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto"><path d="M0,0 L10,5 L0,10 z" fill="#7c879b"/></marker></defs>' +
    [['UNKNOWN', 20, 70, '#b39bff'], ['GRANTED', 160, 20, '#4fd8b6'], ['REVOKED', 300, 70, '#ff8a5c'], ['EXPIRED', 160, 120, '#f2c46d']].map(function (n) { return '<rect x="' + n[1] + '" y="' + n[2] + '" width="100" height="30" rx="15" fill="#121821" stroke="' + n[3] + '"' + (n[0] === 'UNKNOWN' ? ' stroke-dasharray="4 3"' : '') + '/><text x="' + (n[1] + 50) + '" y="' + (n[2] + 19) + '" fill="' + n[3] + '" font-size="11" font-weight="700" text-anchor="middle" font-family="JetBrains Mono">' + n[0] + '</text>'; }).join('') +
    '<path d="M120,78 L160,42" stroke="#7c879b" marker-end="url(#smA)"/><text x="104" y="52" fill="#7c879b" font-size="9">opt in</text>' +
    '<path d="M260,40 L300,72" stroke="#7c879b" marker-end="url(#smA)"/><text x="284" y="48" fill="#7c879b" font-size="9">revoke</text>' +
    '<path d="M300,94 C260,110 250,40 262,36" fill="none" stroke="#7c879b" stroke-dasharray="3 3" marker-end="url(#smA)"/><text x="232" y="92" fill="#7c879b" font-size="9">re-consent</text>' +
    '<path d="M210,50 L210,118" stroke="#7c879b" marker-end="url(#smA)"/><text x="214" y="88" fill="#7c879b" font-size="9">scope ends</text>' +
    '<path d="M160,135 C110,150 90,110 170,52" fill="none" stroke="#7c879b" stroke-dasharray="3 3" marker-end="url(#smA)"/><text x="72" y="150" fill="#7c879b" font-size="9">re-prompt</text>' +
    '<text x="20" y="118" fill="#7c879b" font-size="9">default = no processing</text></svg>';
}
...
V['privacy/retention'] = { title: 'Retention', render: function () {
  ...
  [[1, '1 d'], [7, '1 wk'], [30, '30 d'], [90, '90 d'], [365, '1 yr'], [1095, '3 yr'], [3650, '10 yr']].forEach(function (t) { var x = xs(t[0]); s += '<line x1="' + x + '" x2="' + x + '" y1="10" y2="' + (H - 22) + '" stroke="#1f2835"/><text x="' + x + '" y="' + (H - 6) + '" fill="#7c879b" font-size="10" text-anchor="middle">' + t[1] + '</text>'; });
  rows.forEach(function (r) {
    if (r[0] === 'h') { s += '<text x="8" y="' + (y + 12) + '" fill="#7c879b" font-size="10" font-family="JetBrains Mono" letter-spacing="1">' + esc(r[1].toUpperCase()) + '</text>'; y += 22; return; }
    var d = P.get(r[1]).obj, rt = d.retention, cy = y + 8;
    s += '<g class="node" data-ent="' + d.id + '" tabindex="0" role="button" aria-label="' + esc(d.name) + '"><text x="' + (x0 - 10) + '" y="' + (cy + 4) + '" fill="#cfd6e1" font-size="11" text-anchor="end">' + esc(d.name.slice(0, 28)) + '</text>';
    if (rt.actual == null) s += '<rect x="' + x0 + '" y="' + (cy - 5) + '" width="' + (W - x0 - 30) + '" height="10" rx="5" fill="none" stroke="#b39bff" stroke-dasharray="4 3"/><text x="' + (x0 + 8) + '" y="' + (cy + 4) + '" fill="#b39bff" font-size="10">UNKNOWN RETENTION</text>';
    else if (rt.actual === 0) s += '<text x="' + x0 + '" y="' + (cy + 4) + '" fill="#7c879b" font-size="10">' + esc(rt.note || 'source lifetime') + '</text>';
    else {
      var over = rt.required > 0 && rt.actual > rt.required;
      if (rt.required > 0) s += '<line x1="' + xs(rt.required) + '" x2="' + xs(rt.actual) + '" y1="' + cy + '" y2="' + cy + '" stroke="' + (over ? '#ff8a5c' : '#4fd8b6') + '" stroke-width="3" stroke-opacity=".5"/><line x1="' + xs(rt.required) + '" x2="' + xs(rt.required) + '" y1="' + (cy - 7) + '" y2="' + (cy + 7) + '" stroke="#4fd8b6" stroke-width="2"/>';
      s += '<circle cx="' + xs(rt.actual) + '" cy="' + cy + '" r="5" fill="' + (over ? '#ff8a5c' : '#4fd8b6') + '"' + (rt.ttl ? '' : ' stroke="#fff" stroke-dasharray="2 2"') + '/>';
      if (rt.required == null) s += '<text x="' + (xs(rt.actual) + 9) + '" y="' + (cy + 4) + '" fill="#b39bff" font-size="10">no requirement</text>';
      else if (over) s += '<text x="' + (xs(rt.actual) + 9) + '" y="' + (cy + 4) + '" fill="#ff8a5c" font-size="10">' + Math.round(rt.actual / rt.required) + '× need</text>';
    }
    s += '</g>'; y += 22;
  });
  s += '</svg>';
...
V['privacy/deletion'] = { title: 'Forget Me', render: function () {
  ...
  var col = { verified: '#4fd8b6', waiting: '#f2c46d', failed: '#ff8a5c', unknown: '#b39bff' };
  function draw(state) {
    ...
    T.forEach(function (t, i) {
      var a = -Math.PI / 2 + i * 2 * Math.PI / T.length, x = cx + R * Math.cos(a), y = cy + R * Math.sin(a), st = state[i], c = st ? col[st] : '#2b3647';
      ...
      s += '<g class="node" data-ent="' + t[0] + '" tabindex="0" role="button" aria-label="' + esc(t[1] + ': ' + (st || 'not started')) + '"><circle cx="' + x.toFixed(1) + '" cy="' + y.toFixed(1) + '" r="7" fill="' + (st && st !== 'pending' ? c : '#161d29') + '" stroke="' + c + '"' + (st === 'unknown' ? ' fill-opacity="0" stroke-dasharray="2 2"' : '') + '/><text x="' + (x + dx).toFixed(1) + '" y="' + (y + dy).toFixed(1) + '" fill="#cfd6e1" font-size="10" text-anchor="' + anchor + '">' + esc(t[1]) + '</text></g>';
    });
    s += '<circle cx="' + cx + '" cy="' + cy + '" r="52" fill="#161d29" stroke="#3a475c"/><text x="' + cx + '" y="' + (cy - 4) + '" fill="#e9edf3" font-size="11" font-weight="700" text-anchor="middle">DELETION</text><text x="' + cx + '" y="' + (cy + 11) + '" fill="#e9edf3" font-size="11" font-weight="700" text-anchor="middle">ORCHESTRATOR</text>';
    el.innerHTML = s + '</svg>';
  }
...
V['privacy/tracking'] = { title: 'Tracking', render: function () {
  ...
  return P.pageHead('Privacy', 'Web & mobile tracking observatory', 'Cookies, storage, advertising IDs, pixels, SDKs, fingerprinting, link decoration, CNAME cloaking, email pixels, server-side forwarding. The pixel or SDK usually behaves exactly as designed; what fails is a review that never asked where the events went.') +
    ...
    '<style>.th th.rot{height:130px;vertical-align:bottom;padding:4px}.th th.rot span{writing-mode:vertical-rl;transform:rotate(180deg);font-size:10px}.th td.tc{text-align:center;min-width:24px;color:var(--dim)}.th td.tc.o{color:var(--info);cursor:pointer}.th td.tc.m{color:var(--med);background:rgba(242,196,109,.1);cursor:pointer}.th td.tc.b{color:var(--exp);background:rgba(255,138,92,.16);cursor:pointer}</style>';
} };

/* ════════════ AI / ML ════════════ */
V['privacy/ai'] = { title: 'AI / ML', render: function () {
  var arch = ['ON DEVICE', 'ISOLATED PRIVATE CLOUD', 'INTERNAL CLOUD', 'THIRD-PARTY MODEL'];
  var AC = { 'ON DEVICE': '#4fd8b6', 'ISOLATED PRIVATE CLOUD': '#72b7ff', 'INTERNAL CLOUD': '#f2c46d', 'THIRD-PARTY MODEL': '#ff8a5c' };
...
function dpLedger() {
  ...
  return '<div class="card"><div class="card-h"><h2 class="sec">Differential privacy — budget ledger</h2><span class="sub">' + esc(D.dataset) + '</span></div>' +
    ...
    '<style>.dpbar{display:flex;height:34px;border-radius:9px;overflow:hidden;border:1px solid var(--line2)}.dps{display:flex;align-items:center;justify-content:center;font:600 11px var(--mono);color:#08231d;background:var(--ctl);border-right:2px solid var(--bg);white-space:nowrap;overflow:hidden}.dps:nth-child(even){background:#3bb89a}.dpl{display:flex;align-items:center;justify-content:center;font:600 11px var(--mono);color:var(--muted);background:repeating-linear-gradient(45deg,var(--panel2) 0 6px,var(--panel) 6px 12px)}</style>';
}
