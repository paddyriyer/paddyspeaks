...
/* ════════════ ENFORCEMENT LADDER ════════════ */
var RUNG_DESC = ['“Don’t log PII.” Easy to write, easy to ignore.', 'A human reads the design.', 'A linter or scanner flags it.', 'The diff is blocked if a sensitive column loses its policy.', 'The query engine refuses reads without a valid purpose.', 'Daily scans: new joins, access spikes, TTL drift.'];
V['assurance/controls'] = { title: 'Enforcement Ladder', render: function (s, q) {
  ...
  var ladder = P.LEVELS.map(function (l, i) {
    ...
    return '<div class="rung" style="margin-top:' + ((5 - i) * 26) + 'px;' + (i === 0 ? 'border-color:rgba(255,138,92,.45)' : i >= 4 ? 'border-color:rgba(79,216,182,.35)' : '') + '"><div class="rn">L' + i + '</div><div class="rt">' + l + '</div><div class="rd">' + RUNG_DESC[i] + '</div><div class="cnt ' + (i === 0 && here.length ? 'bad' : i >= 4 ? 'ok' : '') + '">' + here.length + '</div>' +
  ...
  var heat = '<div class="heat" style="grid-template-columns:minmax(110px,150px) repeat(6,minmax(0,1fr))"><span></span>' + P.LEVELS.map(function (l, i) { return '<span class="hh">L' + i + '</span>'; }).join('') + domains.map(function (d) { return '<span class="hr">' + esc(d) + '</span>' + P.LEVELS.map(function (l, i) { var n = NS.controls.filter(function (c) { return c.domain === d && c.level === i; }).length; return '<button class="hc" aria-label="' + esc(d + ' L' + i + ': ' + n) + '" data-go="assurance/controls?d=' + encodeURIComponent(d) + '" style="background:' + (n ? (i === 0 ? 'rgba(255,138,92,.5)' : 'rgba(79,216,182,' + (0.15 + i * 0.1) + ')') : 'var(--panel)') + ';color:' + (n ? '#0b0f15' : 'var(--muted)') + '">' + (n || '') + '</button>'; }).join(''); }).join('') + '</div>';
  var n = NS.controls.length, byL = P.LEVELS.map(function (l, i) { return NS.controls.filter(function (c) { return c.level === i; }).length; });
  return P.pageHead('Assurance', 'Privacy control enforcement ladder', 'Move privacy from documents into machines. Every rung up is harder to build and harder to bypass. Rule of thumb: every hard gate should trace back to a real postmortem.') +
    '<div class="stat-row card" style="margin-bottom:14px">' + [['Only in documents', byL[0], 'bad'], ['Human-enforced', byL[1], ''], ['Static checks', byL[2], ''], ['Deployment gates', byL[3], ''], ['Runtime', byL[4], 'ok'], ['Continuously audited', byL[5], 'ok'], ['Machine-enforced', P.pct(byL[2] + byL[3] + byL[4] + byL[5], n) + '%', ''], ['Failing', NS.controls.filter(function (c) { return c.health === 'failing'; }).length, 'bad']].map(function (x) { return '<div class="stat"><div class="sv ' + x[2] + '">' + x[1] + '</div><div class="sl">' + x[0] + '</div></div>'; }).join('') + '</div>' +
...
V['assurance/audits'] = { title: 'Audits', render: function () {
  ...
  return P.pageHead('Assurance · the auditor', 'Verify what actually happens', 'Privacy bugs are usually assumption bugs. SPEC (what should happen) ≠ IMPLEMENTATION (what happens) ≠ ABUSE / EDGE (what else can happen). Test these every time.') +
    '<div class="grid g4" style="margin-bottom:14px">' + T.map(function (t) { return '<button class="card" data-ent="' + esc(t.ent) + '" style="text-align:left;cursor:pointer;border-color:' + (t.result === 'fail' ? 'rgba(255,138,92,.45)' : t.result === 'unknown' ? 'rgba(179,155,255,.45)' : 'rgba(79,216,182,.35)') + (t.result === 'unknown' ? ';border-style:dashed' : '') + '"><div class="card-h" style="margin-bottom:4px"><b>' + esc(t.k) + '</b><span class="tag ' + (t.result === 'fail' ? 'sev-HIGH' : t.result === 'unknown' ? 'k-UNKNOWN' : 'sev-GOOD') + '">' + t.result.toUpperCase() + '</span></div><div class="small dim">' + esc(t.q) + '</div><div class="small" style="margin-top:6px">' + esc(t.where) + '</div></button>'; }).join('') + '</div>' +
...
V['assurance/access'] = { title: 'Access & Insider', render: function () {
  ...
  return P.pageHead('Assurance', 'Access & insider privacy', 'People with access, services with access, privileged and break-glass use, sensitive queries, exports, large joins and identity-resolution jobs. Encryption does not stop anyone the system decrypts for.') +
    ...
    '<div class="card"><div class="card-h"><h2 class="sec">Who can read what</h2><span class="sub">people with access (log scale)</span></div><div class="bars">' + ds.map(function (d) { var w = Math.log10(d.accessPeople + 1) / Math.log10(max + 1) * 100; return '<div class="br"><span>' + chip(d.id) + '</span><span class="track"><span class="fill" style="width:' + w + '%;background:' + (d.accessPeople > 100 ? 'var(--exp)' : P.dsTier(d) >= 3 && d.accessPeople > 30 ? 'var(--med)' : 'var(--t1)') + '"></span></span><span class="mono small" style="text-align:right">' + d.accessPeople.toLocaleString() + '</span></div>'; }).join('') + '<div class="br"><span>' + chip('d_pulseinstall') + '</span><span class="track" style="border:1px dashed var(--unk)"></span><span class="mono small unknown" style="text-align:right;border:0">unknown</span></div></div></div></div>';
} };
...
V['assurance/incidents'] = { title: 'Incidents', render: function () {
  return P.pageHead('Assurance', 'Privacy incident center', 'Incidents beyond security breaches: exposure, excess access, consent failure, purpose violation, tracking, retention, deletion, vendor misuse, identity linking, logging leaks, misdirected exports, model leakage. Every postmortem ends in one question: can this become a machine-checkable invariant?') +
    NS.incidents.map(function (i) {
      return '<div class="card" style="margin-bottom:12px"><div class="card-h"><div><span class="mono small dim">' + i.id + ' · ' + esc(i.type) + '</span><h2 class="sec" style="margin-top:2px"><button class="chip" data-ent="' + i.id + '" style="font-size:14px;font-weight:650;color:var(--text)">' + esc(i.title) + '</button></h2></div><div class="small"><span class="tag ' + (i.status === 'closed' ? 'sev-GOOD' : 'sev-HIGH') + '">' + esc(i.status) + '</span> <span class="mono dim">' + esc(i.sev) + ' · ' + fmtN(i.people) + ' people · ' + esc(i.opened) + '</span></div></div><div class="chain">' +
        [['Incident', i.title, 'bad'], ['Failed assumption', i.assumption, 'bad'], ['Control gap', i.gap, 'bad'], ['Root cause', i.root, ''], ['Remediation', i.remediation, ''], ['Permanent guard', i.guard, 'good']].map(function (x) { return '<div class="cn ' + x[2] + '"><div class="cl">' + x[0] + '</div><div class="cv">' + esc(x[1]) + '</div></div>'; }).join('') + '</div></div>';
    }).join('');
} };

/* ════════════ DRIFT ════════════ */
V['assurance/drift'] = { title: 'Drift', render: function () {
  var types = P.uniq(NS.drift.map(function (d) { return d.type; }));
    ...
    NS.regulations.map(function (r) {
      return '<div class="card" style="margin-bottom:12px"><div class="card-h"><h2 class="sec">' + chip(r.id) + '</h2><span class="sub">' + esc(r.scope) + '</span></div>' + r.obligations.map(function (o) {
        ...
        return '<div class="chain" style="margin-bottom:4px">' +
          ...
          '<div class="cn"><div class="cl">Systems</div><div class="cv">' + o.systems + ' systems</div></div>' +
          ...
          '<div class="cn ' + (ev ? 'good' : 'unk') + '"><div class="cl">Evidence</div><div class="cv">' + (ev ? esc(ev.evidence) : 'none') + '</div></div>' +
          '<div class="cn ' + (o.status === 'met' ? 'good' : 'bad') + '"><div class="cl">Status</div><div class="cv"><b>' + o.status.toUpperCase() + '</b> ' + esc(o.note) + '</div></div></div>';
      }).join('') + '</div>';
    }).join('');
} };
...
V['governance/vendors'] = { title: 'Vendor Register', render: function () {
  ...
  return P.pageHead('Governance', 'Vendor privacy register', 'For any partner integration, name the handshake pattern first — token, relay, proof, permission or aggregate — then the data, the retention and the contract. “None” means the partner receives the person.') +
    '<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Vendor</th><th>Handshake</th><th>Tier</th><th class="num">People</th><th>Retention c/a</th><th>Contract</th><th>Deletion</th><th>Opt-out</th><th>Privacy review</th><th>Detections</th></tr></thead><tbody>' +
    I.map(function (x) { var v = x.v, hs = NS.handshakes[v.id]; return '<tr class="click" data-ent="' + v.id + '" tabindex="0"><td><b>' + esc(v.name) + '</b><div class="small dim">' + esc(v.role) + '</div></td><td><span class="tag ' + (hs === 'NONE' ? 'sev-HIGH' : 'sev-GOOD') + '">' + hs + '</span></td><td>' + P.tier(v.tier) + '</td><td class="num">' + fmtN(v.people) + '</td><td class="mono small">' + (v.retention.contract == null ? '?' : v.retention.contract) + ' / ' + (v.retention.actual == null ? '?' : v.retention.actual) + '</td><td class="small">' + (v.contract ? esc(v.contract.expires) : unk('none')) + '</td><td>' + (v.deletionApi ? '<span class="ok">API</span>' : '<span class="bad">none</span>') + '</td><td>' + (v.optOutPropagates ? '<span class="ok">yes</span>' : '<span class="bad">no</span>') + '</td><td class="small">' + (v.privacyReview || unk('never')) + '</td><td class="small">' + (x.issues.length ? x.issues.map(function (i) { return '<span class="tag ' + (/unknown/.test(i) ? 'k-UNKNOWN' : 'sev-HIGH') + '" style="margin:1px">' + esc(i) + '</span>'; }).join(' ') : '<span class="ok">none</span>') + '</td></tr>'; }).join('') + '</tbody></table></div>';
} };

/* ════════════ MATURITY ════════════ */
V['governance/maturity'] = { title: 'Maturity', render: function () {
  var L = NS.maturityLevels;
  return P.pageHead('Governance', 'Privacy maturity', 'Not one childish number. Eighteen capabilities, each placed on a five-step scale from ad hoc to verifiable, with the reason.') +
    '<div class="tbl-wrap"><table class="tbl mat"><thead><tr><th>Capability</th>' + L.map(function (l) { return '<th>' + l + '</th>'; }).join('') + '<th>Why this level</th></tr></thead><tbody>' + NS.maturity.map(function (m) { return '<tr><td><b>' + esc(m[0]) + '</b></td>' + L.map(function (l, i) { return '<td class="mc">' + (i < m[1] ? '<i class="past"></i>' : i === m[1] ? '<i class="now" title="' + l + '"></i>' : '<i></i>') + '</td>'; }).join('') + '<td class="small muted">' + esc(m[2]) + '</td></tr>'; }).join('') + '</tbody></table></div>' +
    ...
    '<style>.mat td.mc{text-align:center}.mat td.mc i{display:inline-block;width:14px;height:14px;border-radius:50%;border:1px solid var(--line3)}.mat td.mc i.past{background:rgba(79,216,182,.35);border-color:transparent}.mat td.mc i.now{background:var(--ctl);box-shadow:0 0 0 4px rgba(79,216,182,.18);border-color:var(--ctl)}</style>';
} };

/* ════════════ REPORTS ════════════ */
var MEMO = {
  'R-01': { issue: 'Customer location history is kept 18 months, readable by 7 services, and flows into advertising audiences.', like: 'Misuse already observed (ads). Breach likelihood moderate; regulatory scrutiny high (sensitive PI).', cons: 'Where people sleep and work, available to anyone who compromises one store — and already shaping ads they never agreed to.',
...
V['report/executive'] = { title: 'Executive Report', render: function (s, q) {
  ...
  rid = rid || 'R-01';
  var r = P.get(rid).obj, m = MEMO[rid], c = P.riskCalc(r), d = P.get(r.asset).obj;
  var memo = '<div class="memo"><div class="mh">Privacy decision memo · Northstar (synthetic) · ' + esc(NS.TODAY) + '</div><h2>' + esc(r.name) + ': decision needed</h2>' +
    ...
    '<div class="dec">DECISION REQUIRED by ' + esc(P.get(r.findings[0]) ? P.get(r.findings[0]).obj.due : 'Friday') + ' — owner: ' + esc(P.name(d.owner) || 'UNASSIGNED') + '</div>' +
...
V['report/engineering'] = { title: 'Engineering Report', render: function () {
  ...
  return P.pageHead('Report', 'Engineering report', 'Open findings by owning team, with the enforcement level each fix must reach. Sorted by the team carrying the most HIGH findings.') +
    Object.keys(byT).sort(function (a, b) { var h = function (k) { return byT[k].filter(function (f) { return f.sev === 'HIGH'; }).length; }; return h(b) - h(a); }).map(function (t) {
      return '<div class="card" style="margin-bottom:12px"><div class="card-h"><h2 class="sec">' + (t === '__none' ? unk('NO OWNER') : chip(t)) + '</h2><span class="sub">' + byT[t].length + ' open</span></div><div class="tbl-wrap"><table class="tbl"><thead><tr><th>Finding</th><th>Sev</th><th>Due</th><th>Must reach</th><th>First step</th></tr></thead><tbody>' + byT[t].map(function (f) { var du = P.daysUntil(f.due); return '<tr class="click" data-ent="' + f.id + '" tabindex="0"><td><span class="mono small">' + f.id + '</span> ' + esc(f.title) + '</td><td>' + P.sev(f.sev) + '</td><td class="small ' + (du < 7 ? 'bad' : '') + '">' + esc(f.due) + '</td><td>' + P.lvl(f.enforcement) + '</td><td class="small">' + esc(f.mitigations[0]) + '</td></tr>'; }).join('') + '</tbody></table></div></div>';
    }).join('');
} };
V['report/audit'] = { title: 'Audit Report', render: function () {
  var T = NS.deletionTargets, cs = NS.consentConsumers, pd = P.personalDatasets();
  var rows = [
    ['Personal data discovered (datasets with owner + tier)', P.pct(pd.filter(function (d) { return d.owner; }).length, pd.length) + '%'],
    ...
    ['Datasets violating TTL', P.pct(P.metric('retviol').items().length, NS.datasets.length) + '%'],
    ...
    ['DP budget remaining (Insights)', (NS.dp.total - NS.dp.releases.reduce(function (s, r) { return s + r.eps; }, 0)) + ' of ' + NS.dp.total]
  ];
...
V['report/legal'] = { title: 'Privacy / Legal Report', render: function () {
  ...
  return P.pageHead('Report', 'Privacy / legal report', 'Obligations mapped to systems, with the gaps engineering must close. Orientation only — not legal advice.') +
    '<div class="stat-row card" style="margin-bottom:14px"><div class="stat"><div class="sv">' + all.length + '</div><div class="sl">mapped obligations</div></div><div class="stat"><div class="sv ok">' + all.filter(function (x) { return x[1].status === 'met'; }).length + '</div><div class="sl">met with evidence</div></div><div class="stat"><div class="sv warn">' + all.filter(function (x) { return x[1].status === 'partial'; }).length + '</div><div class="sl">partial</div></div><div class="stat"><div class="sv bad">' + all.filter(function (x) { return x[1].status === 'gap'; }).length + '</div><div class="sl">gaps</div></div></div>' +
    '<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Regime</th><th>Obligation</th><th>Data</th><th>Gap</th><th>Status</th></tr></thead><tbody>' + gaps.map(function (x) { return '<tr><td>' + chip(x[0].id) + '</td><td>' + esc(x[1].text) + '</td><td>' + chip(x[1].data) + '</td><td class="small">' + esc(x[1].note) + '</td><td><span class="tag ' + (x[1].status === 'gap' ? 'sev-HIGH' : 'sev-MEDIUM') + '">' + x[1].status + '</span></td></tr>'; }).join('') + '</tbody></table></div>';
} };
})();
