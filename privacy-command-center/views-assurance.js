...
V['assurance/controls'] = { title: 'Enforcement Ladder', render: function (s, q) {
  ...
  var ladder = P.LEVELS.map(function (l, i) {
    ...
    return '<div class="rung" style="margin-top:' + ((5 - i) * 26) + 'px;' + (i === 0 ? 'border-color:rgba(192,71,15,.45)' : i >= 4 ? 'border-color:rgba(11,125,96,.35)' : '') + '"><div class="rn">L' + i + '</div><div class="rt">' + l + '</div><div class="rd">' + RUNG_DESC[i] + '</div><div class="cnt ' + (i === 0 && here.length ? 'bad' : i >= 4 ? 'ok' : '') + '">' + here.length + '</div>' +
...
V['assurance/audits'] = { title: 'Audits', render: function () {
  ...
  return P.pageHead('Assurance · the auditor', 'Verify what actually happens', 'Privacy bugs are usually assumption bugs. SPEC (what should happen) ≠ IMPLEMENTATION (what happens) ≠ ABUSE / EDGE (what else can happen). Test these every time.') +
    ...
    '<div class="card" style="margin-bottom:14px"><div class="card-h"><h2 class="sec">Replay it as a story</h2><span class="sub">each failure step by step in the essay, with the habits that prevent it</span></div><div class="fx" style="flex-wrap:wrap;gap:8px">' + T.filter(function (t) { return t.essay; }).map(function (t) { return '<a class="chip" href="../articles/every-arrow-is-a-decision.html#' + esc(t.essay) + '">' + esc(t.k) + ' &rarr;</a>'; }).join('') + '</div></div>' +
