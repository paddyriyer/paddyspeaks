...
(function () {
  ...
  function initContact() {
    ...
    // /contact/?reason=mentoring preselects a reason — only one the select offers
    try {
      var want = new URLSearchParams(location.search).get('reason');
      var sel = form.elements.reason;
      if (want && sel && sel.querySelector('option[value="' + want.replace(/[^a-z_]/g, '') + '"]')) sel.value = want;
    } catch (e) {}
