...
(function () {
  ...
  function recordContinue() {
    ...
    if (!meta) return;
    withState(function () { record(meta); });
  }
  ...
  function record(meta) {
    try {
      window.PSState.recordVisit({
        ...
        title: meta.getAttribute('data-title') || document.title.replace(/\s*[|—–-]\s*PaddySpeaks.*$/, ''),
        url: location.pathname + (meta.hasAttribute('data-keep-hash') ? location.hash : ''),
