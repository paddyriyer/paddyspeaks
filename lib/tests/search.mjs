...
/* ── the brief's example queries ── */
{
  ...
  ok(r[0].id === 'concept:karma-yoga', '"karma without attachment" → the Karma yoga concept first');
  ok(r.slice(1, 6).every((d) => d.type === 'verse'), '… then verses');
...
{
  const r = top('spark skew', 4);
  ok(r[0] && r[0].id === 'concept:data-skew', '"spark skew" → the Data skew concept first');
  ok(r[1] && r[1].type === 'topic' && /skew/i.test(r[1].title), '… then a skew design topic');
  ok(r.some((d) => /spark/i.test(d.title)), '"spark skew" → the Spark track in the top 4');
}
