 ...
 * First-party and cookie-free. The Worker stores no IP address.
 *
 * What it is, stated precisely (the public version is /privacy-policy/):
 *   - It keeps a random visitor id (_ps_vid) in localStorage, so return visits
 *     can be counted. That id, with screen size, language and the approximate
 *     location Cloudflare derives from the connection, is a PSEUDONYMOUS
 *     identifier — not a name, but not "anonymous" either. We do not combine
 *     it with anything else or sell it.
 *   - It is switched off entirely by Global Privacy Control (here in the
 *     browser, and by the Worker for the no-JS pixel via the Sec-GPC header).
 *   - Clearing this site's storage resets the id.
 *
 ...
 *   - Global Privacy Control respect (GPC — legal signal, CCPA)
...
(function () {
  ...
  // ── Consent guards ──
  // Respect Global Privacy Control (legal signal, California CCPA).
  // DNT is deprecated (W3C abandoned the spec) and enabled by default in
  // many browsers — honoring it silently drops 15-25% of real traffic.
  // No cookies; the visitor id is a random UUID in localStorage (see header).
  if (navigator.globalPrivacyControl === true) { window.psTrack = function () {}; return; }
  ...
  // ── Search keyword from the referring URL, when a search engine still puts
  // one there (rare today). It IS stored, as page_views.search_query, and is
  // disclosed in /privacy-policy/. It is never text typed on this site. ──
  var searchQuery = '';
  try {
