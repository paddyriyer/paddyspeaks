 ...
 *   - search stays reachable: an existing .nav-search-btn sits beside the toggle
 *     on mobile rather than being buried inside the collapsed menu, and returns
 *     to the nav row on desktop, where the mobile bar is display:none
 */
(function () {
  'use strict';
  ...
  // Search is a first-class action at both sizes, so the control travels rather
  // than being duplicated (one element, one id, one listener). On mobile it sits
  // beside the toggle instead of being buried in the collapsed menu; on desktop
  // it must go back into the nav row, because .ps-nav-mobile-bar is display:none
  // above the breakpoint and a button parked there is simply gone.
  function placeSearch() {
    if (!search) return;
    var target = isMobile() ? bar : nav;
    if (search.parentNode === target) return;
    // A page may mark the link search sits before (the homepage keeps About
    // last and quiet); otherwise search goes to the end of the row.
    var before = target === nav ? nav.querySelector('[data-ps-after-search]') : null;
    target.insertBefore(search, before);
  }
  ...
  // Shared header (lib/ps-chrome.css): once the masthead has scrolled away,
  // mark the sticky row so it shows the small wordmark and a hairline shadow.
  var wrap = document.getElementById('ps-navwrap');
  var mast = document.querySelector('.masthead');
  if (wrap && mast && 'IntersectionObserver' in window) {
    new IntersectionObserver(function (es) {
      wrap.classList.toggle('is-stuck', !es[0].isIntersecting);
    }).observe(mast);
  }
