// Sri Suktam - Interactive Application
// ==========================================
...
(function () {
  ...
  function getTypeLabel(type) {
    switch (type) {
      ...
      case 'glory': return 'Glory of Sri';
  ...
  function renderReading() {
    ...
    html += '<div class="section-divider">Invocation of Sri (1&ndash;5)</div>';
    data.verses.forEach(function (v) {
      if (v.type === 'invocation') html += buildVerseCard(v, 'ri');
    });

    html += '<div class="section-divider">Glory of Sri (6&ndash;10)</div>';
    data.verses.forEach(function (v) {
      if (v.type === 'glory') html += buildVerseCard(v, 'rg');
    });

    html += '<div class="section-divider">Prayers for Blessings (11&ndash;15)</div>';
    data.verses.forEach(function (v) {
      if (v.type === 'blessing') html += buildVerseCard(v, 'rb');
    });

    html += '<div class="section-divider">Phala Shruti (16&ndash;19)</div>';
    data.verses.forEach(function (v) {
      if (v.type === 'phala') html += buildVerseCard(v, 'rp');
    });
